...
namespace ermia
{
  ...
  public:
    ...
    static const uint16_t kIndexConcurrentDash = 0x2;
    ...
    inline void CreateDashTable(const char *name, const char *primary_name = nullptr)
    {
      CreateTable(kIndexConcurrentDash, name, primary_name);
    }
  ...
  class ConcurrentDashIndex : public OrderedIndex
  {
  ...
  private:
    ...
    // nullptr as value.
    rc_t DoDashPut(transaction &t, const varstr *k, varstr *v, bool expect_new,
                   bool upsert, OID *inserted_oid);

  public:
    ConcurrentDashIndex(std::string name, const char *primary)
    ...
    {
      return DoDashPut(*t, &key, &value, false, true, nullptr);
    }
    inline rc_t Insert(transaction *t, const varstr &key, varstr &value,
                       OID *out_oid = nullptr) override
    {
      return DoDashPut(*t, &key, &value, true, true, out_oid);
    }
    inline rc_t Insert(transaction *t, const varstr &key, OID oid) override
    {
      return DoDashPut(*t, &key, (varstr *)&oid, true, false, nullptr);
    }
    inline rc_t Remove(transaction *t, const varstr &key) override
    {
      return DoDashPut(*t, &key, nullptr, false, false, nullptr);
    }
