...
public:
  ...
  inline bool search(const key_type &k, value_type &o, epoch_num e,
  ...
  public:
     ...
     */
    virtual bool invoke(const string_type &k, value_type oid, uint64_t version) = 0;
  };
   ...
   */
  inline bool insert(const key_type &k, value_type o, epoch_mgr::epoch_num e,
   ...
   */
  inline bool insert_if_absent(const key_type &k, OID o, epoch_mgr::epoch_num e,
                               insert_info_t *insert_info = NULL);
...
template <typename P>
inline bool mbtree<P>::search(const key_type &k, value_type &o, epoch_num e,
...
template <typename P>
inline bool mbtree<P>::insert(const key_type &k, value_type o, epoch_mgr::epoch_num e,
...
public:
  ...
  bool visit_oid(const Masstree::key<uint64_t> &key, value_type oid) {
    if (this->boundary_compar_) {
      lcdf::Str bs(this->boundary_->data(), this->boundary_->size());
      if ((!Reverse && bs <= key.full_string()) ||
          (Reverse && bs >= key.full_string()))
...
mbtree<P>::search_range_oid(const key_type &lower, const key_type *upper,
  ...
  // XXX(jianqiuz): Seems I can just pass nullptr to xc, since
  // the xc is not used in the following functions, and we already
  // have threadinfo for epoch management
