...
std::map<std::string, uint64_t> base_txn_btree::unsafe_purge(bool dump_stats) {
  purge_tree_walker w;
...
void base_txn_btree::purge_tree_walker::on_node_begin(
    const typename concurrent_btree::node_opaque_t *n) {
  ASSERT(spec_values.empty());
  spec_values = concurrent_btree::ExtractValues(n);
}

void base_txn_btree::purge_tree_walker::on_node_success() {
  spec_values.clear();
}

void base_txn_btree::purge_tree_walker::on_node_failure() {
  spec_values.clear();
}
...
void base_txn_btree::txn_search_range_callback::on_resp_node(
...
bool base_txn_btree::txn_search_range_callback::invoke(
...
void base_txn_btree::do_search_range_call(transaction &t, const varstr &lower,
                                          const varstr *upper,
...
void base_txn_btree::do_rsearch_range_call(transaction &t, const varstr &upper,
                                           const varstr *lower,
                                           search_range_callback &callback) {
  t.ensure_active();
