...
//uint64_t merge_time;
using DashClsAllocator = ermia::str_arena;
...
constexpr size_t stashMask = stashBucket - 1;
...
struct Bucket {
  inline void prefetch() const {
    for (int i = 0; i < sizeof(Bucket<K, V>); i += CACHELINE_SIZE)
  ...
  inline void set_indicator(uint8_t meta_hash, Bucket<K, V> *neighbor,
                            uint8_t pos) {
  ...
  inline void unset_indicator(uint8_t meta_hash, Bucket<K, V> *neighbor, K key,
                              uint64_t pos) {
  ...
  PROMISE(int) unique_check(uint8_t meta_hash, K key, Bucket<K, V> *neighbor,
                   Bucket<K, V> *stash) {
    V value;
  ...
  bool check_and_get(uint8_t meta_hash, K key, bool probe, V* value) {
    int mask = 0;
  ...
  int Insert(K key, V value, uint8_t meta_hash, bool probe) {
  ...
  int Delete(K key, uint8_t meta_hash, bool probe) {
  ...
  int Insert_with_noflush(K key, V value, uint8_t meta_hash, bool probe) {
    auto slot = find_empty_slot();
  ...
  void Insert_displace(K key, V value, uint8_t meta_hash, int slot,
                       bool probe) {
    _[slot].value = value;
    _[slot].key = key;
    set_hash(slot, meta_hash, probe);
  ...
  void Insert_displace_with_noflush(K key, V value, uint8_t meta_hash,
                                    int slot, bool probe) {
    _[slot].value = value;
    _[slot].key = key;
    set_hash(slot, meta_hash, probe);
...
struct Directory {
  ...
  static void New(Finger_EH<K, V> *ht, Directory **dir, size_t capacity, size_t _version){
    int ret = ht->aligned_alloc((void **)dir, sizeof(Directory<K, V>) + sizeof(table_p) * capacity);
    ALWAYS_ASSERT(ret == 0);
    memset(*dir, 0, sizeof(Directory<K, V>) + sizeof(table_p) * capacity);
    new (*dir) Directory(capacity, _version);
  }

  inline void prefetch_dir_entry(size_t segment_id) const {
    ::prefetch((const void *)(&_[segment_id]));
  }
};
...
template <class K, class V>
struct Table {
  static void New(Finger_EH<K, V> *ht, Table<K, V> **tbl, size_t depth, Table<K, V>* pp) {
    int ret = ht->aligned_alloc((void **)tbl, sizeof(Table<K, V>));
    ALWAYS_ASSERT(ret == 0);
    memset(*tbl, 0, sizeof(Table<K, V>));
  ...
  PROMISE(int) Insert(K key, V value, size_t key_hash, uint8_t meta_hash,
             Directory<K, V> **);
  void Insert4split(K key, V value, size_t key_hash, uint8_t meta_hash);
  void Insert4splitWithCheck(K key, V value, size_t key_hash,
  ...
  int Next_displace(Bucket<K, V> *target, Bucket<K, V> *neighbor,
                    Bucket<K, V> *next_neighbor, K key, V value,
                    uint8_t meta_hash) {
  ...
  int Prev_displace(Bucket<K, V> *target, Bucket<K, V> *prev_neighbor,
                    Bucket<K, V> *neighbor, K key, V value,
                    uint8_t meta_hash) {
  ...
  int Stash_insert(Bucket<K, V> *target, Bucket<K, V> *neighbor, K key, V value,
                   uint8_t meta_hash, int stash_pos) {
    for (int i = 0; i < stashBucket; ++i) {
      Bucket<K, V> *curr_bucket =
  ...
  Bucket<K, V> bucket[kNumBucket + stashBucket];
  size_t local_depth;
  size_t pattern;
  int number;
  Table<K, V> *next;
...
template <class K, class V>
PROMISE(int) Table<K, V>::Insert(K key, V value, size_t key_hash, uint8_t meta_hash,
                     Directory<K, V> **_dir) {
  /*we need to first do the locking and then do the verify*/
  auto y = BUCKET_INDEX(key_hash);
  Bucket<K, V> *target = bucket + y;
  Bucket<K, V> *neighbor = bucket + ((y + 1) & bucketMask);
  target->prefetch();
  neighbor->prefetch();
  SUSPEND;
  target->get_lock();
  ...
  auto ret =
      AWAIT target->unique_check(meta_hash, key, neighbor, bucket + kNumBucket);
  if (ret == -1) {
    neighbor->release_lock();
    target->release_lock();
  ...
  if (((GET_COUNT(target->bitmap)) == kNumPairPerBucket) &&
    ...
    Bucket<K, V> *next_neighbor = bucket + ((y + 2) & bucketMask);
    Bucket<K, V> *prev_neighbor = bucket + ((y + bucketMask) & bucketMask);
    next_neighbor->prefetch();
    prev_neighbor->prefetch();
    SUSPEND;
    // Next displacement
    ...
    Bucket<K, V> *stash = bucket + kNumBucket;
    for (int i = 0; i < stashBucket; ++i) {
      Bucket<K, V> *stash =
          bucket + kNumBucket + ((i + (y & stashMask)) & stashMask);
      stash->prefetch();
    }
    SUSPEND;
...
template <class K, class V>
Table<K, V> *Table<K, V>::Split(Finger_EH<K, V> *ht, size_t _key_hash) {
  ...
  // XXX(shiges): the allocation of this new segment will memset it to zero,
  // obviating the need to prefetch and suspend
  Table<K, V>::New(ht, &next, local_depth + 1, next);
  //Table<K, V> *next_table = reinterpret_cast<Table<K, V> *>(pmemobj_direct(next));
...
template <class K, class V>
class Finger_EH : public Hash<K, V> {
 public:
  Finger_EH(DashClsAllocator *, size_t);
  ~Finger_EH(void);
  inline PROMISE(bool) Insert(K, V);
  inline PROMISE(bool) Delete(K);
  inline PROMISE(bool) Get(K, V *);
  ermia::coro::generator<bool> coro_Insert(K, V);
  ermia::coro::generator<bool> coro_Delete(K);
  ermia::coro::generator<bool> coro_Get(K, V *);
  ...
  int FindAnyway(K key);
  void ShutDown() {
    clean = true;
  }
  void getNumber() {
    std::cout << "The size of the bucket is " << sizeof(struct Bucket<K, V>) << std::endl;
    size_t _count = 0;
    size_t seg_count = 0;
    ...
    ss = reinterpret_cast<Table<K, V> *>(reinterpret_cast<uint64_t>(dir_entry[0]) &
                                      tailMask);
  ...
  inline int aligned_alloc(void **mem, size_t size) {
    assert(allocator);
    char *block = allocator->next_raw_aligned(CACHELINE_SIZE, size);
    *mem = (void *)block;
    return 0;
  }
  ...
  bool clean;
  Directory<K, V> *back_dir;
  DashClsAllocator *allocator;
};

template <class K, class V>
Finger_EH<K, V>::Finger_EH(DashClsAllocator *allocator, size_t initCap) : allocator(allocator) {
  Directory<K, V>::New(this, &dir, initCap, 0);
  lock = 0;
  crash_version = 0;
  clean = false;

  /*FIXME: make the process of initialization crash consistent*/
  Table<K, V>::New(this, &dir->_[initCap - 1], dir->global_depth, NULL);
  dir->_[initCap - 1]->pattern = initCap - 1;
  dir->_[initCap - 1]->state = 0;
  /* Initilize the Directory*/
  for (int i = initCap - 2; i >= 0; --i) {
    Table<K, V>::New(this, &dir->_[i], dir->global_depth, dir->_[i+1]);
    dir->_[i]->pattern = i;
    dir->_[i]->state = 0;
  }
...
template <class K, class V>
Finger_EH<K, V>::~Finger_EH(void) {
  // TO-DO
}

template <class K, class V>
void Finger_EH<K, V>::Lock_Directory() {
  while (!Acquire()) {
    asm("nop");
  }
}
...
template <class K, class V>
void Finger_EH<K, V>::Unlock_Directory() {
  while (!Release()) {
    asm("nop");
  }
}
...
template <class K, class V>
void Finger_EH<K, V>::Halve_Directory() {
  auto d = dir->_;

  Directory<K, V> *new_dir;
  Directory<K, V>::New(this, &new_dir, pow(2, dir->global_depth - 1), dir->version + 1);
...
template <class K, class V>
void Finger_EH<K, V>::Directory_Doubling(int x, Table<K, V> *new_b, Table<K, V> *old_b) {
  Table<K, V> **d = dir->_;
  auto global_depth = dir->global_depth;
  auto capacity = pow(2, global_depth);
  Directory<K, V>::New(this, &back_dir, 2 * capacity, dir->version + 1);
  Directory<K, V> *new_sa = back_dir;
  auto dd = new_sa->_;
...
template <class K, class V>
void Finger_EH<K, V>::Directory_Update(Directory<K, V> *_sa, int x, Table<K, V> *new_b,
                                    Table<K, V> *old_b) {
...
template <class K, class V>
void Finger_EH<K, V>::Directory_Merge_Update(Directory<K, V> *_sa, uint64_t key_hash,
                                          Table<K, V> *left_seg) {
  Table<K, V> **dir_entry = _sa->_;
  auto global_depth = _sa->global_depth;
...
template <class K, class V>
PROMISE(bool) Finger_EH<K, V>::Insert(K key, V value) {
  uint64_t key_hash;
  if constexpr (std::is_pointer<K>::value) {
...
RETRY:
  ...
  if (ret == -1) {
    ...
    // Prefetch the entire segment
    for (int i = 1; i < kNumBucket; ++i) {
      (target->bucket + i)->prefetch();
    }
    SUSPEND;
    auto new_b =
        target->Split(this, key_hash); /* also needs the verify..., and we use try
  ...
  REINSERT:
    ...
    if (target->local_depth < old_sa->global_depth) {
      ...
      // XXX(shiges): It looks like prefetching those directory entries
      // to be updated are not very helpful.
...
template <class K, class V>
PROMISE(bool) Finger_EH<K, V>::Get(K key, V *value) {
  uint64_t key_hash;
  if constexpr (std::is_pointer<K>::value) {
    key_hash = h(key->key, key->length);
  } else {
...
RETRY:
  ...
  Bucket<K, V> *neighbor_bucket = target->bucket + ((y + 1) & bucketMask);

  // TODO: 
  // - add prefetch and suspend here
  // - use vtune
  target_bucket->prefetch();
  neighbor_bucket->prefetch();
  SUSPEND;
...
template <class K, class V>
void Finger_EH<K, V>::TryMerge(size_t key_hash) {
...
template <class K, class V>
PROMISE(bool) Finger_EH<K, V>::Delete(K key) {
...
RETRY:
  ...
  /*we need to first do the locking and then do the verify*/
  auto y = BUCKET_INDEX(key_hash);
  Bucket<K, V> *target = target_table->bucket + y;
  Bucket<K, V> *neighbor = target_table->bucket + ((y + 1) & bucketMask);
  target->prefetch();
  neighbor->prefetch();
  SUSPEND;
  target->get_lock();
  ...
  TEST_STASH:
    if (test_stash == true) {
      Bucket<K, V> *stash = target_table->bucket + kNumBucket;
      stash->get_lock();
      for (int i = 0; i < stashBucket; ++i) {
        int index = ((i + (y & stashMask)) & stashMask);
        Bucket<K, V> *curr_stash = target_table->bucket + kNumBucket + index;
...
template <class K, class V>
ermia::coro::generator<bool> Finger_EH<K, V>::coro_Insert(K key, V value) {
  uint64_t key_hash;
  if constexpr (std::is_pointer<K>::value) {
    key_hash = h(key->key, key->length);
  } else {
...
RETRY:
  ...
  // auto ret = AWAIT target->Insert(key, value, key_hash, meta_hash, &dir);
  // start: Table<K, V>::Insert
  int table_insert_retval = 0xFFFF;
  {
#define TI_RETURN(v) do { table_insert_retval = (v); goto finish_table_insert; } while (0)
    Bucket<K, V> *bucket = target->bucket;
    Directory<K, V> **_dir = &dir;
    /*we need to first do the locking and then do the verify*/
    auto y = BUCKET_INDEX(key_hash);
    Bucket<K, V> *target_bucket = bucket + y;
    Bucket<K, V> *neighbor = bucket + ((y + 1) & bucketMask);
    target_bucket->prefetch();
    neighbor->prefetch();
    co_await suspend_always{};
    ...
    // auto ret =
    //     AWAIT target->unique_check(meta_hash, key, neighbor, bucket + kNumBucket);
    // start: Bucket<K, V>::unique_check
    int bucket_unique_check_retval = 0xFFFF;
    {
#define BU_RETURN(v) do { bucket_unique_check_retval = (v); goto finish_bucket_unique_check; } while (0)
      Bucket<K, V> *stash = bucket + kNumBucket;
      V value;
      if ((target_bucket->check_and_get(meta_hash, key, false, &value) == true) ||
          (neighbor->check_and_get(meta_hash, key, true, &value) == true)) {
        BU_RETURN(-1);
      }
      ...
      if (target_bucket->test_stash_check()) {
        ...
        if (target_bucket->test_overflow()) {
          ...
          if (mask != 0) {
            for (int i = 0; i < 4; ++i) {
              if (CHECK_BIT(mask, i) && (target_bucket->finger_array[14 + i] == meta_hash) &&
                  (((1 << i) & target_bucket->overflowMember) == 0)) {
                test_stash = true;
                goto STASH_CHECK;
              }
      ...
      STASH_CHECK:
        if (test_stash == true) {
          ...
          co_await suspend_always{};
          for (int i = 0; i < stashBucket; ++i) {
            Bucket<K, V> *curr_bucket = stash + i;
            if (curr_bucket->check_and_get(meta_hash, key, false, &value) == true) {
              BU_RETURN(-1);
            }
          }
...
#undef BU_RETURN
    ...
    // end: Bucket<K, V>::unique_check
  finish_bucket_unique_check:
    assert(bucket_unique_check_retval != 0xFFFF);
    // auto ret =
    //     AWAIT target->unique_check(meta_hash, key, neighbor, bucket + kNumBucket);
    auto ret = bucket_unique_check_retval;
    if (ret == -1) {
      neighbor->release_lock();
      target_bucket->release_lock();
      TI_RETURN(-3); /* means this key this key already exist*/
    }

    if (((GET_COUNT(target_bucket->bitmap)) == kNumPairPerBucket) &&
        ((GET_COUNT(neighbor->bitmap)) == kNumPairPerBucket)) {
      Bucket<K, V> *next_neighbor = bucket + ((y + 2) & bucketMask);
      Bucket<K, V> *prev_neighbor = bucket + ((y + bucketMask) & bucketMask);
      next_neighbor->prefetch();
      ...
      auto ret =
          target->Next_displace(target_bucket, neighbor, next_neighbor, key, value, meta_hash);
      if (ret == 0) {
        TI_RETURN(0);
      }
      next_neighbor->release_lock();
      ...
      ret = target->Prev_displace(target_bucket, prev_neighbor, neighbor, key, value, meta_hash);
      if (ret == 0) {
        TI_RETURN(0);
      }
      ...
      ret = target->Stash_insert(target_bucket, neighbor, key, value, meta_hash, y & stashMask);
    ...
    * probe bucket are released
    */
    if (GET_COUNT(target_bucket->bitmap) <= GET_COUNT(neighbor->bitmap)) {
...
#undef TI_RETURN
  ...
  // end: Table<K, V>::Insert
finish_table_insert:
  assert(table_insert_retval != 0xFFFF);
  // auto ret = AWAIT target->Insert(key, value, key_hash, meta_hash, &dir);
  auto ret = table_insert_retval;
...
template <class K, class V>
ermia::coro::generator<bool> Finger_EH<K, V>::coro_Get(K key, V *value) {
  uint64_t key_hash;
  if constexpr (std::is_pointer<K>::value) {
    key_hash = h(key->key, key->length);
  } else {
...
template <class K, class V>
ermia::coro::generator<bool> Finger_EH<K, V>::coro_Delete(K key) {
  // Not implemented
  ALWAYS_ASSERT(false);
  co_return false;
}
...
template <class K, class V>
int Finger_EH<K, V>::FindAnyway(K key) {
  uint64_t key_hash;
  if constexpr (std::is_pointer<K>::value) {
