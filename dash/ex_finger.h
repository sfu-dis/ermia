...
#define GET_BITMAP(var) ((var) >> 18)
  ...
  struct Bucket
  {
    ...
    int unique_check(uint8_t meta_hash, K key, Bucket<K, V> *neighbor,
                     Bucket<K, V> *stash)
    {
      V value;
      ...
      STASH_CHECK:
        if (test_stash == true)
        {
          for (int i = 0; i < stashBucket; ++i)
          {
            ...
            if (curr_bucket->check_and_get(meta_hash, key, false, &value) ==
                true)
            {
              return -1;
            }
    ...
    void Insert_displace_with_noflush(K key, V value, uint8_t meta_hash, int slot,
                                      bool probe)
    {
      _[slot].value = value;
      _[slot].key = key;
  ...
  struct Directory
  {
    ...
    {
      Allocator::ZAllocate((void **)dir, kCacheLineSize,
                           sizeof(Directory<K, V>) + sizeof(table_p) * capacity);
  ...
  struct Table
  {
    ...
    int Insert(K key, V value, size_t key_hash, uint8_t meta_hash,
               Directory<K, V> **);
    ...
    int Prev_displace(Bucket<K, V> *target, Bucket<K, V> *prev_neighbor,
                      Bucket<K, V> *neighbor, K key, V value, uint8_t meta_hash)
    {
  ...
  template <class K, class V>
  class Finger_EH : public ermia::Hash<K, V>
  {
  public:
    Finger_EH(size_t);
    ~Finger_EH(void);
    inline int Insert(K key, V value);
    int Insert(K key, V value, bool);
    inline bool Delete(K);
    bool Delete(K, bool);
    inline bool Get(K, V *);
    ...
    void getNumber()
    {
      std::cout << "The size of the bucket is " << sizeof(struct Bucket<K, V>)
                << std::endl;
      size_t _count = 0;
      size_t seg_count = 0;
  ...
  template <class K, class V>
  Finger_EH<K, V>::Finger_EH(size_t initCap)
  {
    Directory<K, V>::New(&dir, initCap, 0);
    lock = 0;
    crash_version = 0;
    clean = false;
  ...
  void Finger_EH<K, V>::Halve_Directory()
  {
    ...
    Directory<K, V>::New(&new_dir, pow(2, dir->global_depth - 1),
                         dir->version + 1);
  ...
  template <class K, class V>
  int Finger_EH<K, V>::Insert(K key, V value, bool is_in_epoch)
  {
    if (!is_in_epoch)
    {
  ...
  template <class K, class V>
  int Finger_EH<K, V>::Insert(K key, V value)
  {
    uint64_t key_hash;
    if constexpr (std::is_pointer<K>::value)
    {
  ...
  template <class K, class V>
  bool Finger_EH<K, V>::Get(K key, V *value, bool is_in_epoch)
  {
    if (!is_in_epoch)
    {
      auto epoch_guard = Allocator::AquireEpochGuard();
  ...
  template <class K, class V>
  bool Finger_EH<K, V>::Delete(K key, bool is_in_epoch)
  {
    if (!is_in_epoch)
    {
      auto epoch_guard = Allocator::AquireEpochGuard();
  ...
  template <class K, class V>
  bool Finger_EH<K, V>::Delete(K key)
  {
